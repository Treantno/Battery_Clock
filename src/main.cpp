...
MAX72XXDisplay  Disp_min(&dispChain, 10, 2);
MAX72XXDisplay  Disp_sec(&dispChain, 8, 2);
MAX72XXDisplay  Disp2(&dispChain, 4, 4);
MAX72XXDisplay  Disp3(&dispChain, 0, 4);
...
//    Disp_sec.writeNumber(reversedigits(seconds), MAX72XX::Character::ZERO);
    Disp_min.writeNumber(minutes, MAX72XX::Character::ZERO);
    Disp_sec.writeNumber(seconds, MAX72XX::Character::ZERO);

    if(half_second >= 500)
      Disp_min.setDecimalPoint(0);
    else
      Disp_min.setDecimalPoint(2);
  }
  else if(restart == 0)
  {
  ...
  Disp3.writeNumber(cnt2 ^ 0xffff, MAX72XX::Character::ZERO);
